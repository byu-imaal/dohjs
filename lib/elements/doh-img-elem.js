...
class DohImg extends HTMLElement {
  ...
  load() {
    return new Promise(((resolve, reject) => {
      ...
      dohElemUtils.dnsLookupResource(imgSrc, dohUrl, method)
        .then(ip => {
          console.log(imgSrc, '-->', ip);
          return dohElemUtils.getResource(imgSrc, ip, this.headers)
        })
        .then(imgBytes => {
  ...
  addImageToDOM(imgBytes) {
    ...
    img.class = this.getAttribute('class');
